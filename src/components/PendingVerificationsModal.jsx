...
export default function PendingVerificationsModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
      <div className="bg-[#0b0e14] border-2 border-[#00f5d4] p-5 sm:p-6 max-w-2xl w-full relative shadow-[0_0_50px_rgba(0,245,212,0.3)] rounded-xl space-y-4 max-h-[90vh] overflow-y-auto no-scrollbar">
        {/* Close Button */}
        <button
          onClick={() => {
            soundFX.playClick();
          ...
          className="absolute top-3 right-3 text-gray-400 hover:text-[#00f5d4] p-1 text-xl font-bold transition-colors"
        >
          ✕
        </button>
        ...
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 px-2.5 py-0.5 bg-[#00f5d4]/10 border border-[#00f5d4]/40 rounded text-[10px] font-mono text-[#00f5d4] uppercase">
          ...
          <h2 className="font-['Outfit'] text-xl sm:text-2xl font-black text-white uppercase tracking-tight flex items-center gap-2">
            ...
            <span className="bg-[#00f5d4] text-black text-xs font-bold px-2 py-0.5 rounded-full">
        ...
        {pendingExhibits.length === 0 ? (
          ...
          <div className="space-y-4">
            {pendingExhibits.map(exhibit => (
              <div
                key={exhibit.id}
                className="bg-[#121620] border border-[#00f5d4]/40 p-4 rounded-lg space-y-3 relative group hover:border-[#00f5d4] transition-all shadow-md"
              >
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2 border-b border-white/10 pb-2">
                  ...
                  <span className="bg-[#ffc700]/10 text-[#ffc700] border border-[#ffc700]/30 font-['Outfit'] text-xs font-black px-2.5 py-1 rounded self-start sm:self-auto">
                    +{exhibit.points} PTS
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center">
                  ...
                  <div className="sm:col-span-7 space-y-2 text-xs">
                    <div className="p-2 bg-[#0b0e14] border border-white/10 rounded font-['Montserrat'] text-gray-300">
                      <span className="text-[#00f5d4] font-bold block mb-0.5">💬 {currentLang === 'hy' ? 'Քաղաքացու նշումը:' : 'User Notes:'}</span>
                      "{exhibit.proofNotes || (currentLang === 'hy' ? 'Աղբը մաքրված է և տեղափոխված աղբաման:' : 'Trash cleaned and disposed of.')}"
                    </div>

                    <div className="flex items-center gap-2 text-[10px] font-mono text-gray-400">
                      <span className="material-symbols-outlined text-xs text-[#00f5d4]">schedule</span>
                ...
                <div className="flex items-center justify-end gap-2 pt-2 border-t border-white/10">
                  ...
                  <button
                    ...
                    className="bg-[#00f5d4] text-[#0b0e14] hover:bg-[#00d0b4] px-4 py-2 text-xs font-['Archivo_Narrow'] font-black uppercase transition-all rounded shadow-md flex items-center gap-1.5"
                  >
