...
export default function MapView({ exhibits, districts, onSelectExhibit, onCleanExhibit, currentLang = 'hy' }) {
  ...
  const [isHotspotListOpen, setIsHotspotListOpen] = useState(false); // Collapsible on mobile
  ...
  return (
    <div className="app-map-view space-y-4 sm:space-y-5 px-1 sm:px-0 py-2 sm:py-4 animate-fadeIn">
      {/* Header */}
      ...
      <div className="relative w-full h-[400px] sm:h-[480px] lg:h-[580px] xl:h-[640px] bg-[var(--bg-main)] border-2 border-[var(--primary-gold)] overflow-hidden rounded-xl shadow-2xl">
        {mapMode === 'yandex' ? (
          ...
          <div className="relative w-full h-full p-4">
            ...
            {/* Map Legend Overlay */}
            <div className="absolute bottom-3 left-3 bg-[var(--bg-deep)]/90 p-2 border border-white/20 text-[10px] space-y-1 backdrop-blur-sm rounded-lg shadow-md pointer-events-none">
              <div className="flex items-center gap-1.5 text-[var(--primary-gold)]">
                <span className="w-2 h-2 rounded-full bg-[var(--primary-gold)] inline-block animate-pulse"></span>
        ...
        {/* Collapsible Hotspot Markers Overlay Button & List */}
        <div className="absolute top-2.5 right-2.5 z-20 flex flex-col items-end">
          <button
            onClick={() => {
              soundFX.playClick();
              setIsHotspotListOpen(!isHotspotListOpen);
            }}
            className="bg-[var(--bg-deep)]/95 text-[var(--primary-gold)] border border-[var(--primary-gold)] px-2.5 py-1.5 rounded-lg text-xs font-['Space_Grotesk'] font-bold flex items-center gap-1.5 shadow-xl backdrop-blur-md hover:bg-[var(--primary-gold)] hover:text-black transition-all"
          >
            <span className="material-symbols-outlined text-sm">location_on</span>
            <span>HOTSPOTS ({filteredExhibits.length})</span>
            <span className="material-symbols-outlined text-sm">
              {isHotspotListOpen ? 'expand_less' : 'expand_more'}
            </span>
          </button>

          {/* Drawer Dropdown Menu */}
          {isHotspotListOpen && (
            <div className="mt-1.5 bg-[var(--bg-deep)]/95 p-2.5 border border-[var(--primary-gold)] rounded-xl w-64 max-h-56 overflow-y-auto custom-scrollbar space-y-1.5 backdrop-blur-md shadow-2xl animate-fadeIn">
              {filteredExhibits.map(ex => (
                <div
                  key={ex.id}
                  onClick={() => {
