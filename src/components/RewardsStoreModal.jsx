...
export default function RewardsStoreModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-fadeIn">
      <div className="bg-[#0b0e14] border-2 border-[#ffc700] p-5 sm:p-6 max-w-3xl w-full relative shadow-[0_0_50px_rgba(255,199,0,0.3)] rounded-xl space-y-4 max-h-[90vh] overflow-y-auto no-scrollbar">
        {/* Close Button */}
        <button
          onClick={() => {
            soundFX.playClick();
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
          ...
          <div className="bg-[#121620] p-3 border-2 border-[#ffc700] rounded-lg text-right shadow-lg self-start sm:self-auto">
            <span className="text-[10px] font-mono text-gray-400 block uppercase">YOUR BALANCE</span>
            <span className="font-['Outfit'] font-black text-[#ffc700] text-lg">
              {userPoints} PTS
            </span>
          </div>
        </div>
        ...
        <div className="flex border-b border-white/10 gap-2">
          <button
            ...
            className={`py-2 px-4 font-['Archivo_Narrow'] text-xs font-bold uppercase transition-all rounded-t border-b-2 ${
              activeTab === 'store'
          ...
          <button
            ...
            className={`py-2 px-4 font-['Archivo_Narrow'] text-xs font-bold uppercase transition-all rounded-t border-b-2 flex items-center gap-1.5 ${
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {initialRewards.map((reward) => {
              ...
              return (
                <div
                  key={reward.id}
                  className="bg-[#121620] border border-white/10 hover:border-[#ffc700] p-4 rounded-lg flex flex-col justify-between space-y-3 transition-all group relative overflow-hidden shadow-md"
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-['Outfit'] text-xs font-black text-[#ffc700] bg-[#ffc700]/10 border border-[#ffc700]/30 px-2.5 py-1 rounded">
                        {reward.ptsCost} PTS
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <h4 className="font-['Outfit'] text-base font-bold text-white uppercase mt-0.5 group-hover:text-[#ffc700] transition-colors">
                        {title}
                      </h4>
                    </div>
                  ...
                  <button
                    ...
                    disabled={!canAfford}
                    className={`w-full py-2.5 font-['Archivo_Narrow'] text-xs font-bold uppercase transition-all rounded flex items-center justify-center gap-1.5 ${
                      canAfford
        ...
        {activeTab === 'my_coupons' && (
          <div>
            {myCoupons.length === 0 ? (
              <div className="p-8 bg-[#121620] border border-white/10 rounded-lg text-center space-y-3">
                <span className="material-symbols-outlined text-5xl text-gray-600">local_activity</span>
              ...
              <div className="space-y-3">
                {myCoupons.map((coupon, idx) => (
                  <div
                    key={idx}
                    className="p-4 bg-[#121620] border-2 border-[#00f5d4] rounded-lg flex flex-col sm:flex-row justify-between sm:items-center gap-3 shadow-lg"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="material-symbols-outlined text-xl text-[#00f5d4]">{coupon.icon}</span>
                      ...
                      <p className="text-xs font-mono text-[#00f5d4]">
                        CODE: <span className="bg-black px-2 py-0.5 border border-[#00f5d4] rounded font-bold">{coupon.couponCode}</span>
                      </p>
                    </div>

                    <button
                      onClick={() => setRedeemedCodeModal(coupon)}
                      className="bg-[#00f5d4] text-black font-['Archivo_Narrow'] font-bold text-xs px-3 py-2 uppercase rounded hover:bg-[#00d0b4] transition-all self-start sm:self-auto"
                    >
        ...
        {redeemedCodeModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-lg animate-fadeIn">
            <div className="bg-[#0b0e14] border-2 border-[#00f5d4] p-6 max-w-sm w-full text-center space-y-4 rounded-xl shadow-[0_0_50px_rgba(0,245,212,0.4)] relative">
              <button
              ...
              <div className="bg-white p-4 rounded-lg inline-block shadow-inner mx-auto">
                <img
              ...
              <div className="p-2.5 bg-[#121620] border border-[#00f5d4]/40 rounded font-mono text-xs text-[#00f5d4]">
                <span>COUPON: </span>
